...
public class SystemManager(World world) : IAvailable
{
	...
	{
		foreach (var system in _systems.Where(s => s.Enabled)) (system as IUpdatableSystem)?.Update(gameTime);
	}
	public void Draw(GameTime gameTime)
	{
		foreach (var system in _systems.Where(s => s.Enabled))  (system as IDrawableSystem)?.Draw(gameTime);
	}
